...
class SmsCache
{
    ...
    {
        $data = $this->handler->hgetall($this->getCodeCacheKey($phone, $type));
    ...
    {
        $data = $this->handler->hgetall($this->getNumCacheKey($phone, $type));
    ...
    protected function setCodeCache(string $phone, string $code, string $type, int $time)
    {
        ...
        // 设置验证码过期时间
        if ($res) {
            $this->handler->expire($key, $this->getValidTime());
        }
    ...
    protected function setNumCache(string $phone, string $type, int $time)
    {
        ...
        // 设置发送次数
        $res = $this->handler->hmset($key, $data);

        // 不存在时设置缓存
        if (! $exists && $res) {
            $this->handler->expire($key, $this->getExpireTime());
        }
